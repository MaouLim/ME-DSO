...
namespace utils {
	...
	/** 
	 * @brief Optimized version for shared_ptr type
	 */
	...
	public:
		...
		typedef const _Tp&                 const_reference;
		typedef std::shared_ptr<_Tp>       pointer;
		typedef std::shared_ptr<const _Tp> const_pointer;
	...
	public:
		 ...
		 *        pop out.
		 */
		void force_to_push(pointer p, value_type& out) {
			std::lock_guard<std::mutex> locker(m_mutex);
			if (this->_full()) { 
				out = *m_container.front();
				m_container.pop_front();
		...
		pointer force_to_push(pointer p) {
			std::lock_guard<std::mutex> locker(m_mutex);
			pointer ptr = nullptr;
			if (this->_full()) { 
				ptr = std::move(m_container.front()); 
				m_container.pop_front();
			}
			m_container.push_back(std::move(p));
			m_empty.notify_one();
		...
		bool wait_and_push_for(pointer p, const std::chrono::duration<_Rep, _Period>& time) {
			std::unique_lock<std::mutex> locker(m_mutex);
			bool not_full =
		...
		pointer wait_and_pop_for(const std::chrono::duration<_Rep, _Period>& time) {
			...
			if (not_empty) {
				pointer ptr = std::move(m_container.front());
				m_container.pop_front();
				m_full.notify_one();
				return ptr;
			}
	...
	/**
	 * @brief version for unique_ptr
	 */ 
	template <typename _Tp, typename _Container>
	class bounded_blocking_queue<std::unique_ptr<_Tp>, _Container> {
		...
		typedef _Container                                    container_type;
		typedef std::unique_ptr<_Tp>                          smart_ptr;
		typedef bounded_blocking_queue<smart_ptr, _Container> self_type;
		typedef typename container_type::size_type            size_type;
	...
	public:
		...
		typedef _Tp&                       reference;
		typedef const _Tp&                 const_reference;
		typedef std::unique_ptr<_Tp>       pointer;
		typedef std::unique_ptr<const _Tp> const_pointer;
